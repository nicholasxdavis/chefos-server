...
document.addEventListener('DOMContentLoaded', () => {
        ...
        authForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (result.success && result.user) {
                    ...
                    const planExpiry = result.user.plan_expiry || null;
                    ...
                    // Subscribe date defaults to today in production
                    ...
                    if (authError && authErrorMessage) {
                        ...
                        authErrorMessage.textContent = result.error || 'Invalid email or password.';
                    }
                }
            } catch (error) {
    ...
    if (createAccountForm) {
        createAccountForm.addEventListener('submit', (e) => {
            ...
            let planType = selectedPlan === 'trial' ? 'trial' : 'pro';
            ...
            try {
                ...
                if (result.success && result.user) {
                    // Auto-login
                    const userPlan = result.user.plan || planType;
                    const planExpiry = result.user.plan_expiry || null;
                    ...
                    if (createAccountError && createAccountErrorMessage) {
                        createAccountError.classList.remove('hidden');
                        createAccountErrorMessage.textContent = result.error || 'Failed to create account. Please try again.';
                    }
                    return;
                }
            } catch (error) {
                ...
                if (createAccountError && createAccountErrorMessage) {
                    createAccountError.classList.remove('hidden');
                    createAccountErrorMessage.textContent = 'Registration failed. Please try again.';
                }
                return;
            }
    ...
    if (localStorageRadio && cloudStorageRadio) {
                ...
                if (email) {
                    try {
                        await fetch('/api/user/settings', {
                            ...
                            body: JSON.stringify({
                                email,
                                key: 'chefos_storage_location',
                                value: 'local'
                            })
                        });
                    } catch (error) {
                        console.error('Error saving storage setting:', error);
                    }
                }
        ...
        cloudStorageRadio.addEventListener('change', async () => {
            if (cloudStorageRadio.checked) {
                ...
                if (typeof storage !== 'undefined' && storage.syncAll) {
                    setTimeout(() => {
                        storage.syncAll().catch(err => console.error('Sync error:', err));
                    }, 2000);
                }
            }
        });
    ...
    if (upgradeToProBtn) {
        upgradeToProBtn.addEventListener('click', async () => {
            ...
            // Upgrade user to pro in backend
            try {
                await fetch('/api/user/plan', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ email: userEmail, plan: 'pro', plan_expiry: null })
                });
            } catch (e) {
